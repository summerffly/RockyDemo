...
// Count Sort
void *CountSort_Func(void *args)
{
    int max = 0;
    for( int i=1; i<=MAXSIZE; i++ )
    {
        if(max < gp_rand_array[i])
            max = gp_rand_array[i];
    }
    printf("MAX of Array: %d\n", max);

    int *tmp_array = NULL;
    tmp_array = (int *)malloc( max * sizeof(int) );
    if(tmp_array == NULL)
        perror("malloc() failed \n");

    for( int i=1; i<=MAXSIZE; i++ )
        tmp_array[gp_rand_array[i]]++;
...
int main(int argc, char **argv, char *env[])
{
    ...
    pthread_create(&tid_sort, NULL, CountSort_Func, NULL);
