...
void *SSSort_Func(void *args)
{
    ...
    for(i=1; i<MAXSIZE; i++)
    {
        ...
        {
            g_cmp_count++;
            if(gp_rand_array[min] > gp_rand_array[j])
                min = j;
        }

        if(min != i)
        {
            tmp = gp_rand_array[min];
            gp_rand_array[min] = gp_rand_array[i];
            gp_rand_array[i] = tmp;
...
int main(int argc, char **argv, char *env[])
{
    ...
    pthread_create(&tid_sort, NULL, SSSort_Func, NULL);
